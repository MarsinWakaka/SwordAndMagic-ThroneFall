...
{
    [CreateAssetMenu(fileName = Skill.SkillID.AisDeltaSlash, menuName = "Character/Ais/Skill/Ais_DeltaSlash")]
    public class AisDeltaSlashConfig : ActiveSkillConfig
    {
        ...
        public AttackSkillData GetSkillData(int level)
        {
            if (level < 1 || level > growthData.Length)
            {
                Debug.LogError($"Invalid skill level: {level}");
    ...
    public class DeltaSlashSkillInstance : ActiveSkillInstance
    {
        ...
        {
            var config = SkillConfigManager.Instance.GetConfig(skillId) as AisDeltaSlashConfig;
            if (config == null)
            {
